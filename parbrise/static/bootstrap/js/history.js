$(document).ready(function() {
    // Gestion responsive
    const mobileToggleBtn = $('<button class="btn btn-primary sidebar-toggle-mobile d-lg-none"><i class="fas fa-bars"></i></button>');
    ...
    // Fermer le sidebar mobile lorsqu'on clique à l'extérieur
    $(document).on('click', function(e) {
        if (!$('.sidebar').is(e.target) && $('.sidebar').has(e.target).length === 0 && !mobileToggleBtn.is(e.target)) {
    ...
    $('.view-details').click(function() {
        ...
        // Ici vous devriez faire une requête AJAX pour récupérer les détails complets
        // Pour cet exemple, nous utilisons des données déjà disponibles dans le DOM
        
        // Exemple avec des données statiques (à remplacer par un appel AJAX)
        var row = $(this).closest('tr');
        
        $('#modalCodeSap').text(row.find('td:eq(2)').text());
        $('#modalModele').text(row.find('td:eq(3)').text());
        $('#modalSimilarity').text(row.find('.progress-bar').text().trim());
        $('#modalBarcode').text(row.find('td:eq(5)').text().trim());
        $('#modalStatus').text(row.find('td:eq(6)').text().trim());
        $('#modalTimestamp').text(row.find('td:eq(0)').text());
        $('#modalUser').text(row.find('td:eq(1)').text());
    });
});
$(document).ready(function() {
    ...
    // Fermer en cliquant en dehors (pour desktop)
